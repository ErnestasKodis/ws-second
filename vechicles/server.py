...
def PatchAVehicle(vehicleId):
    vehicle = [vehicle for vehicle in vehicles if vehicle["id"] == vehicleId]
    data = request.get_json("force=True")
    ...
    message = ""
    if "model" in data:
        vehicle[0]["model"] = data["model"]
        message += "model "
    if "year" in data:
        vehicle[0]["year"] = data["year"]
        message += "year "
    if "origin" in data:
        vehicle[0]["origin"] = data["origin"]
        message += "origin "
    if "phone" in data:
        vehicle[0]["phone"] = data["phone"]
        message += "phone "
...
if __name__ == "__main__":
    server.run(host="0.0.0.0", port=6969, debug=True)
